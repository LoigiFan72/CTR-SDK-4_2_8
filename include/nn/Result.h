...
{
private:
    static const bit32 MASK_FAIL_BIT        = 0x80000000u; // Most Significant Bit
    ...
    //NN_STATIC_ASSERT( SHIFTS_LEVEL + SIZE_LEVEL == 32 );

    static const bit32 MASK_NEGATIVE_LEVEL  = (~0u) << SIZE_LEVEL;

#define NN_RESULT_H_MAKE_MASK(size, shift)  (((~0u) >> (32 - (size))) << (shift))
#define NN_RESULT_H_MAKE_MASK_HELPER(c)     \
    static const bit32 MASK_ ## c           = NN_RESULT_H_MAKE_MASK(SIZE_ ## c, SHIFTS_ ## c)
...
#define NN_RESULT_H_MAKE_MAX_HELPER(c)      \
    static const s32 MAX_ ## c              = NN_RESULT_H_MAKE_MAX(SIZE_ ## c)
...
public:
    ...
    enum Description
    {
        ...
        DESCRIPTION_INVALID_RESULT_VALUE            = MAX_DESCRIPTION -  0   //
    };
...
private:
    ...
    explicit Result(bit32 code) : m_Code(code) {}

public:
    Result()
        : m_Code( static_cast<bit32>(
    ...
    Result(nnResult result) : m_Code(result.value) {}
/* Please see man pages for details 
...
*/
    ...
    {
        // TODO: Should create a platform-dependent arithmetic shift inline function and call it
        if(m_Code & MASK_FAIL_BIT)
        {
    ...
    {
        return static_cast<Summary>( GetCodeBits(MASK_SUMMARY, SHIFTS_SUMMARY) );
    }
    ...
    {
        return static_cast<Module>( GetCodeBits(MASK_MODULE, SHIFTS_MODULE) );
    }
    ...
    {
        return static_cast<int>( GetCodeBits(MASK_DESCRIPTION, SHIFTS_DESCRIPTION) );
    }

    bit32 GetValue() const { return m_Code; }
/* Please see man pages for details 
...
*/
     ...
     */
    bit32 GetPrintableBits() const { return m_Code; }
...
struct Result::ConstRange : public Result
{
    ...
    static bool Includes(Result result)
    {
        return result.GetModule() == TModule && (TDescriptionMin <= result.GetDescription() && result.GetDescription() <= TDescriptionMax);
    }
...
inline Result MakeInfoResult(Result::Summary summary, Result::Module module, int description)
    { return Result(Result::LEVEL_INFO, summary, module, description); }
inline Result MakeFatalResult(Result::Summary summary, Result::Module module, int description)
    { return Result(Result::LEVEL_FATAL, summary, module, description); }
inline Result MakeResetResult(Result::Summary summary, Result::Module module, int description)
    { return Result(Result::LEVEL_RESET, summary, module, description); }
inline Result MakeReInitResult(Result::Summary summary, Result::Module module, int description)
    { return Result(Result::LEVEL_REINIT, summary, module, description); }
inline Result MakeUsageResult(Result::Summary summary, Result::Module module, int description)
    { return Result(Result::LEVEL_USAGE, summary, module, description); }
inline Result MakePermanentResult(Result::Summary summary, Result::Module module, int description)
    { return Result(Result::LEVEL_PERMANENT, summary, module, description); }
inline Result MakeTemporaryResult(Result::Summary summary, Result::Module module, int description)
    { return Result(Result::LEVEL_TEMPORARY, summary, module, description); }
inline Result MakeStatusResult(Result::Summary summary, Result::Module module, int description)
