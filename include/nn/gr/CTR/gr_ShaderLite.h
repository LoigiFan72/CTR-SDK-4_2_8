...
#ifndef NN_GR_SHADERLITE_H_
#define NN_GR_SHADERLITE_H_
    ...
    namespace gr
    {
        namespace CTR
        {
            ...
            public :
                ...
                {
                    std::memcpy( command, &m_CmdCacheConstArray[ m_ConstRgCmdOffsetArray[m_VtxShaderIndex] ],
                ...
                {
                    std::memcpy( command, &m_CmdCacheConstArray[ m_ConstRgCmdOffsetArray[m_GeoShaderIndex] ], 
                ...
                {
                    // ASSERT if the geometry shader mode and render mode do not match
                    NN_GR_ASSERT( ( ! IsEnableGeoShader() ) || ( m_DrawMode == PICA_DATA_DRAW_GEOMETRY_PRIMITIVE ) );

                    // Generate command according to the triangle render mode
                    // 0x229 [8:8]
                    ...
                    *command++ = PICA_CMD_HEADER_SINGLE_BE( PICA_REG_DRAW_MODE1, 0x2 );
                    ...
                    std::memcpy( command, &m_CmdCacheOutAttrArray[ m_CmdCacheOutAttrOffsetArray[ m_CombShaderIndex ] ],
                                 m_CmdCacheOutAttrNumArray[ m_CombShaderIndex ] * sizeof( bit32 ) );

                    return command + m_CmdCacheOutAttrNumArray[ m_CombShaderIndex ];
                }

            public : // Gets information from the binary
                /* Please see man pages for details 
