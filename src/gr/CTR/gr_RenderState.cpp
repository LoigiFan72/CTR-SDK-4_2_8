...
namespace nn
{
    namespace gr
    {
        namespace CTR
        {
            ...
            {
                // Send a dummy command to 0x080 three times
                ...
                // Clear the lowest 1 bit and use the upper 23 bits of the 24 bits                
                u32 zBiasFix24   = Float32ToUnsignedFix24( zBias );
                zBiasFix24       = zBiasFix24 >> 1;

                // The zScale value is invalid and kept just for compatibility.
                // (Assigned 0 at this point. )
                u8 zScale8 = 0;
                
                // 0x8b
                *command++ = 
                    PICA_CMD_DATA_TEXTURE_SHADOW( isPerspective,
                        // f32 -> 24bit unsigned, fixed point upper 23 bits
                        zBiasFix24,
                        // zScale value is invalid
                        zScale8 );
            ...
            {
                // [ 0:7 ] and [12:12] in 0x107 are depth test settings, and [8:11] is the color mask settings.
                ...
                if ( wScale == 0.0f )
                {
                    ...
                    *command++ = PICA_CMD_HEADER_SINGLE( PICA_REG_FRAGOP_WSCALE_DATA1 );
                    ...
                    f32 zNear = isEnablePolygonOffset ? 
                        // To prevent the value from disappearing completely due to errors at the limits of 24-bit float precision, the value is multiplied by 128.0f as a reference depth value in the neighborhood of 0.5f.
                        // 
            ...
            bit32* RenderState::FBAccess::MakeCommand( bit32* command, bool isClearFrameBufferCache ) const
            {
                ...
                else
                {
                    ...
                    *command++ = ( (m_RenderState.colorMask && m_RenderState.colorMask != 0xf) || 
                                   (m_RenderState.colorMask && m_RenderState.blend.isEnable )  || 
                                   (m_RenderState.colorMask && m_RenderState.logicOp.isEnable ) ) ? 0xf : 0;
                    *command++ = PICA_CMD_HEADER_SINGLE_BE( PICA_REG_COLOR_BUFFER_READ, 0x1 );
