    ...
    namespace gr
    {
        namespace CTR
        {
                                            ...
                                            const PicaDataVertexAttrType type )
            {
                ...
                // Disabled because it may be set somewhere else
                DisableAttr_( bind_reg );

                // Search for an empty stream
            ...
            {    
                ...
                {
                    // Disable fixed vertex attributes
                ...
                // Disable if searching in the load array
                for ( int i = 0; i < VERTEX_ATTRIBUTE_MAX; ++i )
                {
                    ...
                    for ( int j = 0; j < VERTEX_ATTRIBUTE_MAX; ++j )
                    {
                        ...
                        // When disabled, checks if the load array itself can be disabled
            ...
            void Vertex::LoadArray::CheckDisable()
            {
                ...
                // When enabled, disable if all content is invalid
            ...
            // #define NN_GR_VERTEX_DUMP
            #if defined( NN_GR_VERTEX_DUMP ) // For debugging
                bit32 * start = command;
            #endif

                // Set the vertex shader input register count later
                ...
                *command++            = 0;
                ...
                for ( int i = 0; i < VERTEX_ATTRIBUTE_MAX; ++i )
                {
                    ...
                    for ( int j = 0; j < VERTEX_ATTRIBUTE_MAX; ++j )
                    {
                        // Consider padding automatically inserted depending on the order of all elements type
                        ...
                        {                            
                            ...
                            // Set the input register index in 0x2bb or 0x2bc
                    ...
                    *command++ = elem[ 0 ];
                    ...
                    // 0x205 + ( 3 * N )
                    // Strides for each vertex is a multiple of the size of the largest type of the vertex attributes included in the vertex data structure
                    // Padding automatically inserted must be considered
                ...
                *headerBaseAddr = PICA_CMD_HEADER_BURSTSEQ( PICA_REG_VERTEX_ATTR_ARRAYS_BASE_ADDR, 3 + 3 * array_num );
                ...
                for ( int bind_reg = VERTEX_ATTRIBUTE_MAX-1; bind_reg >= 0; --bind_reg )
                {
                    ...
                    // Index to input to the vertex shader
                    *command++ = input_index;                    
                    // 0x232
                    *command++ = PICA_CMD_HEADER_BURSTSEQ( PICA_REG_VS_FIXED_ATTR , 4 );
                    // The 3 data portions of the fixed vertex attributes converted to 24 bit: 0x233, 0x234, 0x235
                    ...
                    *command++ = 0; // Padding
                    ...
                    // 0x202[27:16] Fixed vertex attribute mask
                    fixed_attr_mask |= ( 1 << input_index + 16 );

                    ++input_index;
                }

                // Vertex attribute input count setting
                // 0x2b9
