...
namespace nn
{
    namespace gr
    {
        namespace CTR
        {
            ...
            {
                // Vertex shader and geometry shader test        
            ...
            void ShaderLite::MakeShaderConstCommandCache_(void)
            {
                ...
                {
                    // Create constant register command cache.
            ...
            void ShaderLite::MakeShaderOutAttrCommandCache_( void )
            {
                ...
                for ( u32 vert_idx = 0; vert_idx < m_ExeImageInfoNum; vert_idx++ )
                {
                    ...
                    {
                        m_GeoShaderIndex = -1;
                        // Generate a command cache for output attributes. (No geometry shader)
                        ...
                        for ( u32 geo_idx = 0; geo_idx < m_ExeImageInfoNum; geo_idx++ )
                        {
                            if( m_ExeImageInfo[geo_idx]->isGeoShader )
                            {
                                ...
                                // Generate a command cache for output attributes. (With geometry shader)
                                MakeOutAttrCommand_( &m_CmdCacheOutAttrArray[ offset ],
                                                     vert_idx,
                                                     m_GeoShaderIndex );
                                offset += m_CmdCacheOutAttrNumArray[ shader_idx ];
            ...
            {
                // Generate commands such as those for configuring geometry shader use
                {
                    command = MakePrepareCommand( command );
                }

                // Generate geometry shader commands
                if ( IsEnableGeoShader() )
                {                     
                ...
                // Generate vertex shader commands
                {
                ...
                // Generate commands related to output attributes (outmap)
                {
            ...
            bit32* ShaderLite::MakeShaderCommand( bit32* command, const bool isMakePrepareCommand ) const
            {
                ...
                // Generate commands for the geometry shader constant registers
                if ( IsEnableGeoShader() )
                {
                    command = MakeGeoConstRgCommand( command );
                    command = MakeGeoBoolMapCommand( command );
                ...
                // Generate commands for the vertex shader constant registers
                {
                    command = MakeVtxConstRgCommand( command );
                    command = MakeVtxBoolMapCommand( command );
                }
            ...
            bit32* ShaderLite::MakeVtxProgramCommand( bit32* command ) const
            {
                ...
                { // Set the program code load address
                    *command++ = 0;
                ...
                { // Notification that program update finished
                    *command++ = 1;
                                                   ...
                                                   const PicaDataDrawMode drawMode )
            {
                ...
                // 0x229[1:0] Sets the geometry shader to enabled or disabled
                {
                ...
                // 0x244 Whether the geometry shader settings are shared with the vertex shader
                { 
            ...
            void ShaderLite::CheckVtxShaderIndex_( const s32 vtx_shader_index )
            {
                ...
                // Geometry shader integrity test
                                                ...
                                                const s32 geo_shader_index )
            {
                ...
                // When the geometry shader is enabled,
                // The output attributes configure the geometry shader
                ...
                // Get output attribute settings
                {
                    ...
                    if ( is_geometry_shader && exe_info->outputMaps )
                    {
                        ...
                        // Merge all output attributes other than generic output attributes that are defined by both VS and GS
                        NN_GR_ASSERT( outMapBufferCount < OUT_ATTR_BUFFER_MAX );
                        for( s32 g = 0; g < exe_info->outMapCount; ++g )
                        {
                            // Exclude GS generic attributes (value 9)
                                 ...
                                 ( outmapInfo[ g ].type != 7 ) )
                            {
                                ...
                                {
                                    // Exclude VS generic attributes (value 9)
                        ...
                        // Next, sets the output attributes defined with only GS
                        for( s32 g = 0; g < exe_info->outMapCount; ++g )
                        {
                            if ( ( !( gs_copy_mask & ( 1 << g ) )) && 
                                 ( outmapInfo[ g ].type >= 0  )    &&
                        ...
                        // Last, sets the output attributes defined with only VS
                        for( s32 v = 0; v < vtx_exe_info->outMapCount; ++v )
                        {
                            if ( ( !( vs_copy_mask & ( 1 << v ) ) ) && 
                ...
                if ( is_geometry_shader )
                {  
                    // Gets the vertex shader output attribute settings
                    //m_DrawMode = PICA_DATA_DRAW_GEOMETRY_PRIMITIVE;
                    ...
                    *command++ = PICA_CMD_HEADER_SINGLE( PICA_REG_VS_OUT_REG_NUM2 );
                    ...
                    // 0x28b Assumes that the vertex shader output is made to match the geometry shader input
                    *command++ = 0x76543210;
                    ...
                    *command++ = PICA_CMD_HEADER_SINGLE( PICA_REG_GS_ATTR_IN_REG_MAP1 );
                    ...
                    // 0x254
                    if ( ( gsDataMode == 1 ) &&
                         ( m_ExeImageInfo[ geo_shader_index ]->gsVertexNum != 0 ) )
                    {
                        *command++ = m_ExeImageInfo[ geo_shader_index ]->gsVertexNum - 1;
                ...
                else
                {
                    ...
                    *command++ = PICA_CMD_HEADER_SINGLE_BE( PICA_REG_DRAW_MODE1, 0x1 );
                    ...
                    // 0x289 [31:24], [15:8], [3:0] Vertex shader mode setting
                    *command++ = 0xa0000000;
                    *command++ = PICA_CMD_HEADER_SINGLE_BE( PICA_REG_GS_ATTR_NUM, 0xb );
            ...
            bool ShaderLite::SearchBindSymbol( BindSymbol* symbol,
            ...
            u32 ShaderLite::SearchBindSymbolNum(
            ...
            bool ShaderLite::SearchBindSymbol(
                BindSymbol* symbol,
            ...
            {
                // Offset value calculation for constant registers
                ...
                // Calculates index array required in the output attribute command cache
                // Based on this array, the index when combining the vertex shader and geometry shader is calculated

                // When m_GeoShaderIndex = -1
                ...
                // Calculates the offset value required in the output attribute command cache
            ...
            u32 ShaderLite::CalculateBufferSize_() const
            {
                ...
                for ( u8 i = 0; i < m_ExeImageInfoNum; i++ )
                {
                    ...
                    // Constant register command count
                ...
                // Vertex shader and geometry shader combination count
                u32 num_geo = 0;
                for(u8 i = 0; i < m_ExeImageInfoNum; i++)
                {
                ...
                for(u8 i = 0; i < m_ExeImageInfoNum; i++)
                {
                    ...
                    {
                        // Operation on the vertex shader alone
                        size += 38 * sizeof(u32);
                        // Combined with the geometry shader
            ...
            void ShaderLite::AssignBuffer_(bit32* buf)
            {
                ...
                for(u8 i = 0; i < m_ExeImageInfoNum; i++)
                {
                    if( !m_ExeImageInfo[i]->isGeoShader )
                    {
                        // Considering operating on the vertex shader alone
                        m_CmdCacheOutAttrNumArray[idx] = 38;
                        idx++;
                        // Considering combining with all the geometry shaders
                        for(int j = 0; j < m_ExeImageInfoNum; j++)
                        {
                            if( m_ExeImageInfo[j]->isGeoShader )
                            {
